...
def enforce_max_chunk_size(chunks: List[str], max_chars: int = 2500) -> List[str]:
    """
    Hard-split any chunk exceeding max_chars on word boundaries.

    Guard for legacy/processed data generated by an older chunker that may have
    produced oversized chunks (which can exceed an embedding API's input limit).
    """
    out: List[str] = []
    for chunk in chunks:
        if len(chunk) <= max_chars:
            out.append(chunk)
            continue
        out.extend(_split_long_text(chunk, max_chars))
